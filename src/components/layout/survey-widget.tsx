...
export function SurveyWidget({ surveys }: Props) {
  ...
  // Auto-open on mount if there are pending surveys
  useEffect(() => {
    if (pending.length > 0 && !autoOpened) {
      setAutoOpened(true);
      openSurvey(pending[0].id);
    }
  }, [pending.length]);
  ...
  return (
    <>
      {/* Header icon */}
      <button
        onClick={() => openSurvey(pending[0].id)}
        className="relative flex items-center justify-center rounded-lg p-1.5 text-ink-500 hover:bg-ink-50 hover:text-ink-700 transition-colors"
